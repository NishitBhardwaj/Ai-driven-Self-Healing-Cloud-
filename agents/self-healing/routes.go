...
// Routes sets up HTTP routes for the Self-Healing Agent
type Routes struct {
...
func NewRoutes(agent *SelfHealingAgent) *Routes {
	return &Routes{
		agent:  agent,
		logger: agent.logger,
	}
...
func (r *Routes) RegisterRoutes(router *mux.Router) {
	...
	router.HandleFunc("/heal", r.HealHandler).Methods("POST")
}
...
func (r *Routes) ActionHandler(w http.ResponseWriter, req *http.Request) {
	...
	response := map[string]interface{}{
		...
		"message": "Healing action will be processed",
		"agent":   r.agent.GetName(),
	}
...
// HealHandler handles healing requests
func (r *Routes) HealHandler(w http.ResponseWriter, req *http.Request) {
	var healingReq HealingRequest
	if err := json.NewDecoder(req.Body).Decode(&healingReq); err != nil {
		http.Error(w, "Invalid healing request format", http.StatusBadRequest)
		return
	}

	result, err := r.agent.healer.Heal(&healingReq)
	if err != nil {
